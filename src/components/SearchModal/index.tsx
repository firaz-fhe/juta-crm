...
const debouncedSearch = debounce(async (query: string, page: number, isNewSearch: boolean) => {
  ...
  const userResponse = await fetch(`https://bisnesgpt.jutateknologi.com/api/user/config?email=${encodeURIComponent(userEmail)}`, {
    method: 'GET',
    headers: {
  ...
  try {
    ...
    const baseUrl = 'https://bisnesgpt.jutateknologi.com'; // or your production base URL
