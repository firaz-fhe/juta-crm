...
const debouncedSearch = debounce(async (query: string, page: number, isNewSearch: boolean) => {
  ...
  const userResponse = await fetch(`https://raucous-joaquin-unexamining.ngrok-free.dev/api/user/config?email=${encodeURIComponent(userEmail)}`, {
    method: 'GET',
    headers: {
  ...
  try {
    ...
    const baseUrl = 'https://raucous-joaquin-unexamining.ngrok-free.dev'; // or your production base URL
