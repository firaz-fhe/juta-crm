...
const AIFollowupBuilder: React.FC<AIFollowupBuilderProps> = ({
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (!apiKey) {
          ...
          try {
            const configResponse = await axios.get(`https://raucous-joaquin-unexamining.ngrok-free.dev/api/company-config/${companyId}`);
  ...
  const fetchCurrentFollowUps = async () => {
    ...
    try {
      ...
      const templatesResponse = await axios.get(
        `https://raucous-joaquin-unexamining.ngrok-free.dev/api/followup-templates?companyId=${encodeURIComponent(companyId)}`
      );
  ...
  const sendMessageToAI = async (messageText: string) => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://raucous-joaquin-unexamining.ngrok-free.dev/api/followup-brainstorm/',
        {
          message: messageText,
          email: userEmail,
  ...
  const applyChangesToFollowUps = async () => {
    ...
    finalValidation.forEach((template: any, index: number) => {
      ...
      const response = await axios.post(
        'https://raucous-joaquin-unexamining.ngrok-free.dev/api/followup-save-templates/',
        {
          companyId: companyId,
          email: userEmail,
        templates: finalValidation
