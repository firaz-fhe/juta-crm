  ...
  async function fetchEmployeeData(employeeId: string): Promise<Employee | null> {
    try {
      ...
      const contactsResponse = await axios.get(
        `http://localhost:8443/api/companies/${companyId}/contacts?email=${encodeURIComponent(userEmail)}`,
        {
          headers: {}
        }
      );
...
setEngagementScore(Number(newEngagementScore.toFixed(2)));
        ...
        if (companyId) {
          ...
          const loadAIDataFirst = async () => {
            try {
              ...
              const configResponse = await axios.get(`http://localhost:8443/api/user-config?email=${encodeURIComponent(userEmail)}`, {
                headers: {
                  'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
              });
  ...
  const fetchAvailableTags = async () => {
    ...
    try {
      ...
      // Get tags data
      const tagsResponse = await axios.get(`http://localhost:8443/api/companies/${companyId}/tags`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  async function fetchCompanyData() {
    try {
      ...
      const dashboardRes = await axios.get(`http://localhost:8443/api/dashboard/${companyId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
    ...
    try {
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          const dashboardResponse = await axios.get(`http://localhost:8443/api/dashboard/${companyId}`, {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
            },
      ...
      try {
        const activityResponse = await axios.get(`http://localhost:8443/api/companies/${companyId}/recent-activity`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
      ...
      try {
        const performanceResponse = await axios.get(`http://localhost:8443/api/companies/${companyId}/performance-metrics`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const fetchEmployeeAssignments = async (companyId: string, userEmail: string) => {
    try {
      ...
      try {
        const assignmentsResponse = await axios.get(`http://localhost:8443/api/assignments/${companyId}`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
    ...
    try {
      ...
      try {
        const response = await axios.get(
          `http://localhost:8443/api/stats/${companyId}?employeeId=${employeeId}`,
          {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
  ...
  const fetchContactsForPeriod = async (periodStart: Date, periodEnd: Date) => {
    try {
      ...
      // Get contacts data
      const contactsResponse = await axios.get(`http://localhost:8443/api/companies/${companyId}/contacts`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const assignContactToEmployee = async (contactId: string, employeeName: string) => {
    try {
      ...
      await axios.post(`http://localhost:8443/api/contacts/${companyId}/${contactId}/tags`, {
        tags: [employeeName]
      }, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const removeContactAssignment = async (contactId: string, employeeName: string) => {
    try {
      ...
      await axios.delete(`http://localhost:8443/api/contacts/${companyId}/${contactId}/tags`, {
