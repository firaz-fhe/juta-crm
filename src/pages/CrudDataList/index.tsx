...
function Main() {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      console.log('API endpoint:', 'https://bisnesgpt.serveo.net/api/login');
      ...
      if (response.ok) {
        ...
        setError(data.error || `Login failed with status ${response.status}. Please try again.`);
      }
    } catch (err: any) {
      ...
      if (err.message.includes('Failed to fetch')) {
        setError("Cannot connect to server. The API endpoint may be down or blocked by CORS.");
      } else if (err.name === 'TypeError') {
        setError("Network error: Unable to reach the login server.");
      } else {
