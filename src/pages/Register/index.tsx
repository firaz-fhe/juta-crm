...
function Main() {
  ...
  const sendVerificationCode = async () => {
    try {
      ...
      const baseUrl = data2.apiUrl || 'http://localhost:8443';
  ...
  const sendNewUserNotification = async (userName: string, userEmail: string, companyName: string) => {
    try {
      ...
      await fetch(`http://localhost:8443/api/v2/messages/text/0210/${notificationPhone}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: message,
  ...
  const handleRegister = async () => {
    try {
      ...
      const userResponse = await axios.post(`http://localhost:8443/api/create-user/${encodeURIComponent(email)}/${encodeURIComponent(formatPhoneNumber(phoneNumber))}/${encodeURIComponent(password)}/1/${newCompanyId}`);
