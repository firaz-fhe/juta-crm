...
const FollowUpsPage: React.FC = () => {
  ...
  const fetchTags = async () => {
    try {
      ...
      const tagsResponse = await fetch(
        `http://localhost:8443/api/companies/${companyId}/tags`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  const uploadDocument = async (file: File): Promise<string> => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8443/api/upload-media",
        formData,
        {
          headers: {
  ...
  const updateMessage = async (messageId: string) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8443/api/followup-templates/${template.templateId}/messages/${messageId}`,
        updateData
      );
  ...
  const deleteTemplate = async (templateId: string) => {
    try {
      ...
      const response = await axios.delete(
        `http://localhost:8443/api/followup-templates/${template.templateId}`
      );
  ...
  const fetchTemplates = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8443/api/followup-templates?companyId=${encodeURIComponent(
          companyId
        )}`
      );
  ...
  const editTemplate = async (templateId: string) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8443/api/followup-templates/${editingTemplate.templateId}`,
        updateData
      );
